...
以一个总长度为 4000 字节，头部长度 20 ，片大小为 1500 的 IP 数据报为例：
...
子网中的每个接口 IP 地址都分为**网络部分**和**主机（接口）部分**。为了在 CIDR 中区分两部分内容，每个子网 IP 都带有一个 **32 位**的**子网掩码（network mask）**。子网掩码的值表示 IP 地址中哪些部分是网络部分，如一个 IP 地址`223.1.1.0/24`中的`/24`就是子网掩码的十进制值形式，表示该 IP 的前 24 位（从左向右，从高位到地位）为网络部分，而后面的 8 位为主机部分，是子网可以自由分配的**地址范围**。

子网掩码的 32 位二进制的**格式**为：值为 1 的部分所对应的 IP 比特为网络部分比特，为 0 的部分对应的 IP 比特为主机部分。如`/24`表示子网掩码`255.255.255.0`，只需将子网掩码和 IP 主机的地址进行**按位和（AND）**运算，计算结果就是网络部分的 IP 地址，**按位非（NOT）**就是子网中的主机地址。
...
在 CIDR 采用前，互联网管理组织使用**分类编址（classful addressing）**的方式按照 8 、16 、24 比特网络部分长度的分类方式发放被称为 A 、B 、C 、类的 IP 地址。一个 A 类地址只能容纳 2^7^ 个（256） IP 地址，一般被发放给小型组织，而 B 类和 C 类则分别可以容纳更多的主机。然而，A 类地址对于中小组织来说并不总是足够，B 、C 地址对于大型和区域组织而言又可能过多，这种分配方式造成了极大的资源浪费，因此提出了 CIDR 分配方式。
...
在 WAN 表中，存储了**实际 IP 地址**和代表不同主机的**套接字端口**的映射关系键值对，NAT 会使用一个实际没有执行套接字作用的端口号用于子网**主机寻址**。在这种情况下，一个 IP 地址实际上对应了**多台主机**接口，而一个端口号则对应一个唯一的子网主机。WAN 表是 IP 端口二元组与主机的映射。在 LAN 表中，存储了扩展的**子网 IP 地址**和主机**进程套接字端口**映射关系键值对。就像在没有 NAT 时一样，经 NAT 路由器转发的分组通过该映射来寻找对应的主机和主机进程。WAN 表中的端口会和 LAN 表中的每个映射相关联，用于响应分组的转换。
