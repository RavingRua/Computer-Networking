...
当发送方需要进行**跨子网**帧传输时，就**不能**直接将子网外节点的目的地 MAC 地址直接放入 ARP 查询分组中，因为查询报文只向子网广播，因此不会得到任何响应。此时，查询分组中的接收方 MAC 地址字段值实际上是子网中**交换机**接口的 MAC 地址。跨子网传输的整个过程大致如下：

1. 发送方子网中的一个节点查询 ARP 表或发送 ARP 查询分组，负责子网内节点互联的交换机接收广播，发现 ARP 分组中的目的地 IP 属于外部子网，因此返回交换机自己的 MAC 地址。发送节点得到的结果是一个**目的 IP-子网交换机接口 MAC** 映射；
2. 发送方节点根据该信息将帧发送至交换机，发送方子网中的路由器接口的 IP 地址被称为**第一跳**交换机 IP 地址。交换机 ARP 模块查询 ARP 表或者在与其他交换机连接的子网中广播 ARP 查询分组，得到的结果是一个**目的 IP-外部子网入口交换机接口 MAC** 映射；
3. 发送方子网交换机向外部子网交换机发送帧，外部子网交换机的 ARP 模块查询 ARP 表或在子网内部广播 ARP 查询分组，得到的结果是**目的 IP-外部子网目的地节点接口 MAC** 映射；
4. 接收方子网交换机向目的节点发送帧，传输结束。
...
如今，以太网已经升级成**基于交换机的星形拓扑结构**，由一个中心节点即**交换机（switch）**转发所有节点的帧，交换机和各节点是**点对点连接**，因此不会发生任何碰撞，且是全双工的，也就**不需要** MAC 协议了（MAC 地址仍用于以太网设备寻址）。
...
由交换机毒化引起的广播问题被称为**广播风暴（broadcast storm）**，指广播数据充斥网络无法处理的情况。广播风暴的原因还可能是链路层交换机没有正确配置而产生了**环路**，导致交换机不断向各接口广播帧，最终导致局域网瘫痪。一个最简单的配置故障是交换机在收到其他交换机发出的 ARP 广播帧、但无法找到需要的 MAC 地址时，向其他局域网交换机广播不必要的 ARP 查询，引发 ARP 查询环路；正确的方式应该是只向该交换机负责的局域网中节点发起 ARP 查询广播，即作流量隔离。

##### 交换机和路由器的比较

在点对点传输中，负责各个局域网流量转发的设备是**交换机**或**路由器**。其中交换机负责两层协议栈内容，使用链路层协议进行转发，是链路层设备；路由器负责三层，使用网络层协议进行转发，是网络层设备；基于 SDN 的现代交换机也能转发网络层数据报。网络管理员在选择局域网互联设备时，应该考虑两者的优缺点。
...
+ 没有流量隔离：交换机会广播所有目的 MAC 地址未知的 IP 数据报到所有接口，这些接口可能通向局域网中的主机，也可能通向其他交换机；当通向其他交换机时，这些交换机又将进行二次广播，因此广播流量将被所有互联节点获取；
...
**多标签交换协议（MPLS）**是建立在网络层设备上的一种基于分组交换的虚拟电路网络。之所以说是虚拟链路层技术，是因为其运作在网络层上，但**并不触及**网络层协议，即不解析 IP 首部字段，不基于 IP 地址进行分组转发，也就不需要执行最长前缀匹配，取而代之的是一组**标签**。**MPLS 首部**被插入在链路层帧首部和网络层数据报首部之间，内部包含一组标签，这些标签告知带有 MPLS 使能的路由器如何选择对应接口。MPLS 技术极大加速了网络层转发速度，已是新一代的 IP 高速骨干网络交换标准。
