...
+ 没有流量隔离：交换机会广播所有目的 MAC 地址未知的 IP 数据报到所有接口，这些接口可能通向局域网中的主机，也可能通向其他交换机；当通向其他交换机时，这些交换机又将进行二次广播，因此广播流量将被所有互联节点获取。如果互联节点中有攻击者，可能会尝试返回一个具有伪造 MAC 的 ARP 响应，将之后的数据流量全部导向攻击者；
...
下面是一个实例，假设整个 Web 页面的请求过程是从主机联网开始的，并且该网络路径中的所有 ARP 表和本地 DNS 服务器对所有信息一无所知。
...
2. DHCP 发现报文所在的 IP 数据报被封装在一个以太网帧中，但是由于不知道当前以太网中交换机的 MAC ，首先有一个 **ARP 广播帧**被发送，目的地 MAC 为 FF-FF-FF-FF-FF-FF。之后主机接收 ARP 响应，填充 ARP 表，向交换机发送 DHCP 报文所在的帧；
3. DHCP 报文由 DHCP 服务器接收并解析，进行后三次 DHCP 握手，主机获取自己的子网 IP 地址。该 DHCP 位于负责子网互联的路由器中，因此该路由器是主机的**网关**，主机将记录第一跳 IP 地址。至此，联网准备完成。
