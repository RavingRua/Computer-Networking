...
ARP 的另一个作用是支持**跨子网**分组传输。一个承载了目的 IP 为外网主机 IP 的帧如果将目的 MAC 直接设置为外网主机的 MAC 地址，毫无疑问该帧不会被任何节点接收，因为该帧必须先通过子网第一跳路由器。因此，承载跨子网数据报的帧的目的 MAC 将是子网路由器接口的 MAC。发送方节点本身并不知道目的 IP 指向外网，因此假如 ARP 表没有目的 IP 记录，会发生以下事情：

1. 发送方主机发送一个 ARP 查询，目的 IP 为外部子网主机 IP，目的 MAC 为广播地址；
2. 第一跳路由器捕获该 ARP 查询，拆封上交给网络层。网络层发现目的 IP 指向外网，因此路由器会返回一个源 MAC 地址为自己的 ARP 响应；
3. 发送方主机接收到 ARP 响应并更新 ARP 表，之后的帧的目的 MAC 均为第一跳路由器接口的地址；
4. 当承载发向外网的数据报的帧到达路由器时，路由器会根据 ARP 表和 IP 转发表选择对应出链路。
